...
ALREADY-TRAINED model saved to ppo_dynamic_v8.zip -- it does NOT retrain.
...
USAGE (Colab, after the model is trained & saved, with `prov` in scope):
    ...
    model = PPO.load("ppo_dynamic_v8")          # reuse the saved model
...
from train_dynamic_v8 import (_always_wait, _repair_on_block, _ifelse_velocity)
...
def _compare_on(cfg, label, model, provider, seeds):
    env = DynamicObstacleEnvV8(cfg, prob_provider=provider)
...
def compare_all(model, provider=None, n=100):
    ...
    ct = DynamicConfigV8(); ct.n_perm_min = 0; ct.n_perm_max = 0
    ...
    cp = DynamicConfigV8(); cp.n_temp_min = 0; cp.n_temp_max = 0
